...
def get_output_filename(test_id:int, date_str:str, 
    ...
    output_filename = os.path.join(
        output_dir, 
        'enr2_shuffled_train_test_score.{iteration_id}.{test_id}.{date_str}.csv'.format(
            test_id = test_id, 
            date_str=date_str,
...
def shuffle_dependent_var(scores:np.array):
...
def get_train_test_score_en_r2(
    phenos:np.array,
    scores:np.array,
    alpha:float, 
    l1_ratio:float
...
shuffled_scores = shuffle_dependent_var(scores)
...
mean_train_test_score = get_train_test_score_en_r2(
    phenos=phenos_r2, 
    scores=shuffled_scores, 
    alpha=alpha_r2, 
