...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="ElasticNetR2 Grid Search Without CV",
    ...
    required.add_argument(
        "-i",
        "--Input",
        help="Seq Selection candidates filename as csv.",
        required=True,
        type=str,
    )
...
def main():
    ...
    output_filename = os.path.join(output_dir,'enr2_gs_run_summary_results_no_cv.{1}.{0}.csv'.format(date_str, test_id))
    gs_details_filename = os.path.join(output_dir,'enr2_gs_details_no_cv.{1}.{0}.csv'.format(date_str, test_id))
