...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="ElasticNetR2 Feature Importance Permutation Test",
...
def format_output_filename(source_filename, sigma_cut_off, test_id, date_str, output_dir):
    ...
    if sigma_cut_off == 0.0:
        results_filename = os.path.join(
            output_dir, 
            'enr2_feature_importance_perm_values.{optimised}{test_id}.{date_str}.csv'.format(
                optimised=optimised,
                test_id=test_id, 
                date_str=date_str
            )
        ...
        plot_filename = os.path.join(
            output_dir,
            "enr2_feature_importance_box_plot.{optimised}{test_id}.{date_str}.png".format(
                optimised=optimised,
                test_id=test_id, 
                date_str=date_str
            )
        ...
        results_filename = os.path.join(
            output_dir,
            'enr2_feature_importance_perm_values_2sig.{optimised}{test_id}.{date_str}.csv'.format(
                optimised=optimised,
                test_id=test_id, 
                date_str=date_str
            )
        ...
        plot_filename = os.path.join(
            output_dir,
            "enr2_feature_importance_box_plot_2sig.{optimised}{test_id}.{date_str}.png".format(
                optimised=optimised,
                test_id=test_id, 
                date_str=date_str
            )
