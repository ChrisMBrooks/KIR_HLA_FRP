...
def format_filename_template(results_filename:str):
    ...
    filename_template = "rf_model_performance_perm_values.{optimised}".format(
        optimised=optimised
    )
...
def format_output_filenames(
    ...
    consolidated_results_filename = os.path.join(
        output_directory, 
        "rf_model_performance_perm_values.{optimised}{test_id}.{date_str}.csv".format(
            optimised=optimised,
            test_id=test_id, 
            date_str=date_str
        )
    ...
    plot_filename = os.path.join(
        output_directory, 
        "rf_model_performance_perm_hist.{optimised}{test_id}.{date_str}.png".format(
            optimised=optimised,
            test_id=test_id, 
            date_str=date_str
        )
    ...
    cdf_results_filename = os.path.join(
        output_directory, 
        "rf_model_performance_cdf_results.{optimised}{test_id}.{date_str}.csv".format(
            optimised=optimised,
            test_id=test_id, 
            date_str=date_str
        )
...
def main():
    ...
    plt.text(model_score, 20, 
        f' mae: {model_score:.4f}, \n p-val: {p_val:.3f}', 
