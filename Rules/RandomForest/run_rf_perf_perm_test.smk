rule run_rf_perf_perm_test:
    input:
        script = "Scripts/RandomForest/rf_perf_perm_test.py",
        test_plan = RF_TEST_PLAN,
        h_params = RF_H_PARAMS_R2,
        features = "Output/{{project}}/RandomForest/{{date_str}}/Test{{test_id}}/rf_seq_selection_candidates.fs_bs.{thresh}.{{test_id}}.{{date_str}}.csv".format(
    ...
    params:
        ...
        n_jobs = 15
    output:
        results = "Output/{project}/RandomForest/{date_str}/Test{test_id}/ParallelisedData/rf_model_performance_perm_values.{iteration_id}.{test_id}.{date_str}.csv",
    log:         
