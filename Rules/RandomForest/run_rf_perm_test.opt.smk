rule run_rf_perm_test_opt:
    input:
        script = "Scripts/RandomForest/run_rf_perm_test.py",
        test_plan = RF_TEST_PLAN,
        h_params = RF_H_PARAMS_R2,
        features = "Output/{project}/RandomForest/{date_str}/Test{test_id}/Optimised/optimised_model_candidates.{test_id}.{date_str}.csv"
    params:
        date_str = "{date_str}",
        test_id = "{test_id}",
        sigma_cut_off = 2,
        out_dir = "Output/{project}/RandomForest/{date_str}/Test{test_id}/Optimised",
        n_jobs = 1
    output:
        results = "Output/{project}/RandomForest/{date_str}/Test{test_id}/Optimised/rf_feature_importance_perm_values_2sig.optimised.{test_id}.{date_str}.csv",
        plot = "Output/{project}/RandomForest/{date_str}/Test{test_id}/Optimised/rf_feature_importance_box_plot_2sig.optimised.{test_id}.{date_str}.png"
    log:         
        file = "Output/log/rf_perm_test.{project}.{test_id}.{date_str}.log.txt"
    threads: 1
    resources:
        mem_mb=1000
    conda: "../../Envs/kir_hla_ml_env.yml"
    shell: 
        """
            python {input.script} \
            --TestPlan {input.test_plan} \
            --HyperParams {input.h_params} \
            --TestID {params.test_id} \
            --SeqSelectionResults {input.features} \
            --DateStr {params.date_str} \
            --SigmaCutOff {params.sigma_cut_off} \
            --OutputDir {params.out_dir} \
            --NumJobs {params.n_jobs} \
            > {log.file}
        """